...
Vertical padding keeps its existing defaults (`top: 42`, `bottom: 58`), which can be overridden independently. With automatic padding enabled, the bottom minimum is `30px`: values below 30 are raised to 30, and larger explicit values are preserved. Titles, single-channel names, and top legends no longer impose a fixed 42px/64px top minimum; they keep their existing drawing positions, so callers using small top padding must allow for possible overlap. Visible Y-axis headers still reserve their measured height plus the existing 8px gap. With `layout.autoPadding: false`, vertical padding is entirely manual. Visible left-axis content reserves a 5px outer inset instead of 2px; right-side calculations and horizontal coordination are unchanged.
...
Y-axis ticks include both exact domain endpoints. Labels automatically share a scientific exponent when the largest absolute domain value is greater than `0` and below `0.001`, or at least `1000`; the unit and exponent appear above each Y axis (unit first on the left, exponent first on the right), aligned to the outer tick-label edges (left-aligned on left axes and right-aligned on right axes), for example `V E+03`, with an 8px gap above the plot. Ordinary axes show only the unit, for example `V`. All ticks show numeric values only. Custom `tickFormat` remains authoritative and suppresses automatic headers. Automatic layout measures headers and ticks, places default axis titles relative to a 12px gap outside the tick labels, with the left title shifted 10px right, and reserves only the top space needed to keep headers inside the SVG; headers do not add a separate row below legends. Explicit title offsets and padding remain supported; default left and right padding are 0px. Automatic layout reserves only visible content plus a 5px left / 2px right outer axis inset; hiding a Y-axis title reclaims its space immediately, while ticks, units, legends, and X-axis metadata still retain the space they need. Explicit `padding` and `layout.horizontalPadding` remain minimums; with `layout.autoPadding: false`, padding is entirely manual. Left headers extend rightward above the plot without increasing left padding; top horizontal legends reserve room for them. Right headers extend leftward over the plot and top horizontal legends avoid both headers. Automatic right padding reclaims up to 10px, limited by visible exterior content and explicit padding minimums. The demo has no horizontal preview inset, so hidden-title tick labels sit close to the container edge.

The zero line defaults to red (`#ff0000`) with `zeroLine.opacity: 0.5`. Set opacity from `0` (transparent) to `1` (opaque); color, width, dash, visibility, and reference axis remain configurable.

`zeroLine.color` accepts CSS colors, including `rgba()` and eight-digit hex (`#RRGGBBAA`). Color alpha multiplies `zeroLine.opacity`, so set `opacity: 1` when alpha is already included in the color:
...
In the demo, open **网格 → 零线** to edit **零线颜色** and **零线透明度**. The color swatch selects an opaque RGB color; its adjacent text input accepts CSS colors with alpha. The separate opacity field applies to either input method.
