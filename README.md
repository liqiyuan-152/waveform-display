...
Y-axis ticks include both exact domain endpoints. Labels automatically share a scientific exponent when the largest absolute domain value is greater than `0` and below `0.001`, or at least `1000`; the exponent and trimmed unit appear above each Y axis, aligned with its tick labels (right-aligned outside left axes, left-aligned outside right axes), for example `E+03 (V)`, with an 8px gap above the plot. Ordinary axes show only the unit, for example `(V)`. All ticks show numeric values only. Custom `tickFormat` remains authoritative and suppresses automatic headers. Automatic layout measures headers and ticks, places default axis titles 12px outside the tick labels, and reserves only the top space needed to keep headers inside the SVG; headers do not add a separate row below legends. Explicit title offsets and padding remain supported; the default left padding is 40px. Headers reserve their full width in the outer margins. SVG width and the original legend positioning and wrapping rules remain unchanged.
