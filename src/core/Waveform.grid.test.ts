...
describe('Waveform grid styling', () => {
  it('uses the shared dashed style and color for both grid axes', () => {
    const { container } = createChart({ grid: { style: 'dashed', color: '#123456' } })
  ...
  it('keeps legacy per-axis color and dash overrides authoritative', () => {
    const { container } = createChart({
      grid: {
        style: 'solid',
        color: '#123456',
        x: { color: '#abcdef', dash: '1 2' },
        y: { color: '#fedcba', dash: '4 5' },
      },
    })
  ...
  it('updates the shared style and color at runtime', () => {
    const { chart, container } = createChart()

    chart.updateOptions({ grid: { style: 'solid', color: '#123456' } })
    ...
    chart.updateOptions({ grid: { style: 'dashed', color: '#654321' } })
